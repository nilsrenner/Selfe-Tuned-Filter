...
In Abbildung \ref{fig:pd_out_freq} sind die Ausgangssignale des Multiplizierers dargestellt. Während das Ausgangssignal der \SI{1000}{\hertz}-Eingänge nur aus einer einzigen Frequenzkomponente besteht, zeigt das Mischsignal der \SI{1100}{\hertz}-Variante (multipliziert mit der \SI{1000}{\hertz}-Referenz) zwei deutliche Hauptkomponenten. Dieses Verhalten lässt sich über die Additionstheoreme beschreiben. Die Multiplikation zweier Sinussignale führt zu einem Produkt mit zwei Frequenzkomponenten. Die hochfrequente Komponente ergibt sich aus der Summe der beiden Eingangsfrequenzen, die niederfrequente Komponente aus der Differenz dieser Frequenzen.\par
\medskip
...
Wie in Abbildung \ref{fig:phase2omega3} zu erkennnen ist, führt die proportionale Frequenzanpassung nicht zum gewünschten Dämpfungseffekt. Sogenannte PI-Regler haben in der Regelungstechnik häufiger dieses Problem, wenn keine zusätzliche Dämpfung integriert wird. Als Lösungsansatz kann das Ausgangssignal des Reglers tiefpassgefiltert werden, danach könnte die Konvergierung zum gewollten Wert besser funktionieren. \cite{philippsenReg}\par
\medskip

Leider führte die Umsetzung des sogenannten PIT-Regler in Python zu keinen neuen Ergebnissen, da kein sich kein stabiler Verlauf ergab. 

Trotz dieser Instabilität soll an dieser Stelle eine Hypothese aufgestellt werden. Die Dauer der Einschwingzeit hängt der Vermutung nach direkt von der anfänglichen Phasen- und Frequenzabweichung ab. Je größer die Differenz zwischen dem Start- und dem Zielwert ist, desto länger benötigt das System, um sich in den Bereich des stabilen Arbeitspunktes zu bewegen und einzuschwingen. Diese Hypothese soll im weiteren Verlauf der Arbeit durch Simulationen oder reale Messungen validiert bzw. widerlegt werden.
...
Im vorliegenden Design wird ein \gls{opv} mit einem \gls{gbw} von \SI{4}{\mega\hertz} verwendet, während der Multiplizierer eine Bandbreite von \SI{10}{\mega\hertz} aufweist. Der \gls{opv} ist somit der limitierende Faktor der Schaltung. Bei Anwendung der Faustformel ergibt sich für das System eine empfohlende maximale Resonanzfrequenz von ca. \SI{40}{\kilo\hertz} bis \SI{400}{\kilo\hertz}. Durch verwendung eines \glspl{opv} mit höherem \gls{gbw} kann der Bereich noch erweitert werden. \par
\medskip

Neben den Parametern des Filters und seiner internen Bauteile limitiert auch die Einschwingzeit der Phasenregelschleife den Frequenzbereich des Systems. Dabei muss der Loop-Filter so dimensioniert werden, dass dieser zum einen schnell genug auf eine Veränderung reagiert, zum anderen aber stabil gegenüber der hochfrequenten Schwingungen bleibt. Die im vorherigen Absatz zeigte Abschätzung gilt also für einen Standardfilter, nicht aber für diesen Self-Tuned Filter.\par
\medskip
...
In der Theorie kann der einstellbare Frequenzbereich über die Gleichung $\omega_0=\frac{\SI{10}{\volt}}{V_c \cdot RC}$ bestimmt werden. Da die Mittenfrequenz in diesem Fall nur über die Steuerspannung abgestimmt werden soll resultiert eine niedrige Spannung in einer hohen Frequenz und umgekert. Die untere Grenze ergibt sich aus der Versorgungsspannung des Integrators im \gls{pd} sowie der anchgeschalteten Widerstandskombination. Dieser Wert wird für eine Versorgungsspannung von \SI{\pm 15}{\volt} höchstens \SI{15}{\volt} betragen. Die obere Grenze wird durch das Rauschen des Multiplizierers, sowie die zunehmende Sensitivität bei kleinen Werten von $V_c$ begrenzt. Da kleine Spannungen im Nenner zu extrem hohen Frequenzen führen, können schon kleine Störungen zu hoher Instabilität führen.\par
\medskip
In der Literatur finden sich nur wenige Angaben zum real erreichbaren Tune-Bereich self-tuned aktiver Filter. Daher soll dies durch eine Simulation oder Messung ermittelt werden. \par
...
Dieser zählt die Anzahl der Nulldurchgänge oder Pulse pro Sekunde und teilt diesen Wert durch 2, sodass die Frequenz in Hertz ermittelt werden kann. Der begrenzende Faktor ist hierbei die Abtastrate der \gls{mcu}. 
...
Die Limitation des digitalen Ansatzes liegt hingegen bei der Systemtaktrate und der Verarbeitungsgeschwindigkeit der \gls{pio}-Module, über welche nur digitale Taktflanken aufgenommen werden.
Durch die Nutzung der \gls{pio}-Pins kann der Systemtakt von \SI{150}{\mega\hertz} verwendet werden, um Flanken direkt in Hardware zu zählen, ohne die CPU zu belasten. In der Theorie liegt die Grenze dabei bei der halben Taktfrequenz, in der Realität wird die Bandbreite durch den vorgeschalteten Komparator, basierend auf einem \gls{opv} mit einem \gls{gbw} von \SI{4}{\mega\hertz}, zusätzlich eingeschränkt. \par
\medskip
